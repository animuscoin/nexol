...
example /usr/src/nexol, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\nexol`, for example:
    ...
    make install DESTDIR=/mnt/c/workspace/nexol
...
causes two of the nexol executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the nexol code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the nexol source code).
