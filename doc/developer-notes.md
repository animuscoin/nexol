...
- [Development guidelines](#development-guidelines)
    - [General Bitcoin Core](#general-nexolcore)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play nexols" on the test network, if you
...
([`valgrind.supp`](https://github.com/nexolcore/nexol/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_nexol
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_nexol --log_level=test_suite
$ valgrind -v --leak-check=full src/nexold -printtoconsole
```
...
# A coverage report will now be accessible at `./test_nexol.coverage/index.html`.
```
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/nexolcore/nexol/issues/12691)
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `NEXOL_FOO_BAR_H`, e.g.

```c++
#ifndef NEXOL_FOO_BAR_H
#define NEXOL_FOO_BAR_H
...
#endif // NEXOL_FOO_BAR_H
```
...
- src/libsecp256k1
  - Upstream at https://github.com/nexolcore/secp256k1/ ; actively maintaned by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/nexolcore/ctaes ; actively maintained by Core contributors.
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:nexol/nexol.git
...
Commit [`bb81e173`](https://github.com/nexolcore/nexol/commit/bb81e173) is an example of a scripted-diff.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `nexol-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `nexol-cli` and the GUI debug console use this table to determine how to
