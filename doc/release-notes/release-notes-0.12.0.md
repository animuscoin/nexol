Bitcoin Core version 0.12.0 is now available from:
  ...
  <https://nexol.org/bin/nexolcore-0.12.0/>
...
[libsecp256k1](https://github.com/nexolcore/secp256k1) instead of OpenSSL.
...
(https://github.com/nexol/bips/blob/master/bip-0130.mediawiki)
...
(https://github.com/nexol/bips/blob/master/bip-0125.mediawiki).
...
This allows running nexold without having to do any manual configuration.
...
Nexol can now (optionally) asynchronously notify clients through a
...
`nexol.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/nexolcore/nexol/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/nexol/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/nexol/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /nexolrpc>
        ...
        # optional bypass nexold rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< nexolrpc:<password>
    </Location>
...
Restarting nexold will no longer clear out the list of banned peers; instead
...
- #6269 `d4565b6` gitian: Use the new nexol-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include nexol-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix nexol-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of nexol-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override NEXOLD and NEXOLCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [nexol-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add NEXOLD_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libnexolconsensus (Braydon Fuller)
