...
{
    if(fLiteMode) return; // ignore all Nexol related functionality
...
/*
    ...
    allow endless transaction that would bloat Nexol and make it unusable. To
...
bool CDarksendPool::MakeCollateralAmounts(const CompactTallyItem& tallyItem)
{
    ...
    if(!fSuccess) {
        ...
        fSuccess = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
                nFeeRet, nChangePosRet, strFail, coinControlNull, true, ONLY_NOT1500IFMN);
        if(!fSuccess) {
            LogPrintf("CDarksendPool::MakeCollateralAmounts -- ONLY_NOT1500IFMN Error: %s\n", strFail);
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(const CTxIn& txin, const CPubKey& pubkey)
{
    ...
    if(GetTransaction(txin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        BOOST_FOREACH(CTxOut out, tx.vout)
			if(out.nValue == 1500*COIN && out.scriptPubKey == payee)
				return true;
    }
...
{
    if(fLiteMode) return; // disable all Nexol specific functionality
    ...
    RenameThread("nexol-privatesend");
