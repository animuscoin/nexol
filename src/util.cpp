...
const char * const BITCOIN_CONF_FILENAME = "nexol.conf";
const char * const BITCOIN_PID_FILENAME = "nexold.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "nexol" is a composite category enabling all Nexol-related debug output
            if(ptrCategory->count(string("nexol"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NexolCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NexolCore
    // Mac: ~/Library/Application Support/NexolCore
    // Unix: ~/.nexolcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NexolCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NexolCore";
#else
    // Unix
    return pathRet / ".nexolcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty nexol.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override nexol.conf
