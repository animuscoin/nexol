...
reference the release tarball available from https://nexol.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bitcoin git repository](https://github.com/nexolcore/nexol) but are not part of
...
Currently only `Source10` is used, for the example `nexol.conf` file.
...
in the nexol git repository. At present only an SVG version of the Bitcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the nexol
...
The `nexol.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `nexol-server` package. At this point, most RPM based
...
The `nexol-server` package should have SELinux support. How to properly do
...
4. I renamed the `nexol` package that contains the Qt GUI to `nexolcore` as
that appears to be how the general population refers to it, in contrast to
`nexol-xt` or `nexol-classic`. I wanted to make sure the general population
