...
WORKINGDIR="/tmp/nexol_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="http://nexolcore.com"
HOST2="http://nexolcore.com"
BASEDIR="/bin/"
VERSIONPREFIX="nexolcore-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'nexol-',
...
if [ $? -ne 0 ]; then
   echo "nexolcore.com failed to provide signature file, but nexolcore.com did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "nexolcore.com signature files were not equal?"
